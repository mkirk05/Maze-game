...
./maze test_data/heightError.txt > tmp;
...
./maze test_data/widthError.txt > tmp;
if grep -q "Error: Maze does not meet requirements" tmp;
then
    echo "PASS"
else
...
./maze test_data/maze_6x7.txt > tmp;
...
echo "" | timeout 0.2 ./maze test_data/maze_6x7.txt > tmp
...
echo "f" | timeout 0.2 ./maze test_data/maze_6x7.txt > tmp
if grep -q "Error: Invalid input" tmp;
then
    echo "PASS"
else
...
echo "w" | timeout 0.2 ./maze test_data/maze_6x7.txt > tmp
...
echo "d" | timeout 0.2 ./maze test_data/win.txt > tmp
...
echo "m" | timeout 0.2 ./maze test_data/maze_6x7.txt > tmp
if grep -q "### ##" tmp;
then
    echo "PASS"
else
...
echo "s" | timeout 0.2 ./maze test_data/maze_10x10 > tmp
